...
static const int64_t BASE_L = 1000000000;
...
static const int64_t powers_of_10[]={1,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000};
...
struct bigint {
    ...
    friend std::pair<bigint, bigint> divmod(const bigint& a1, const bigint& b1) {
        ...
        for (int32_t i = a.digits.size() - 1; i >= 0; i--) {
            r *= BASE;
            r += a.digits[i];
    ...
    void multiply_fft(const std::vector<int32_t> &x, const std::vector<int32_t> &y, std::vector<int32_t> &res) const {
        ...
        int32_t n = 1;
        while (n < (int32_t) std::max(x.size(), y.size()))
            n <<= 1;
        n <<= 1;
        fa.resize(n);
        fb.resize(n);
    ...
    bigint mul_karatsuba(const bigint &v) const {
        std::vector<int32_t> x6 = convert_base(this->digits, BASE_DIGITS, 6);
    ...
    friend bigint sqrt(const bigint &a1) {
        ...
        for(int32_t j = n / 2 - 1; j >= 0; j--) {
            for(; ; --q) {
                bigint r1 = (r - (res * 2 * bigint(BASE) + q) * q) * bigint(BASE) * bigint(BASE) + (j > 0 ? (int64_t) a.digits[2 * j - 1] * BASE + a.digits[2 * j - 2] : 0);
                if (r1 >= 0) {
                    r = r1;
                    break;
                }
    ...
    std::string to_string() const {
        ...
        std::string temp;
        for(int32_t i=digits.size()-2;i>=0;i--){
            temp=std::to_string(digits[i]);
            for(int32_t j=0;j<9-temp.size();j++){
                ans+='0';
            }
            ans+=temp;
        }   
    ...
    bigint& operator++(){
        ...
        if(sign==1){
        digits[0]++;
        for(int32_t i=0;i<digits.size()-1;i++){
    ...
    bigint& operator>>=(int32_t n){
        ...
        while(n>9){
            ...
            for(int32_t i=1;i<digits.size();++i){
    ...
    bigint& operator<<=(int32_t n){
        while(n>29){
            ...
            if(carry){
                digits.push_back((int32_t)carry);
            }
            n-=29;
        }
        int64_t carry=0;
